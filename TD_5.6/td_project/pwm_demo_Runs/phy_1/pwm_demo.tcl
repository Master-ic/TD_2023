...
set rc [catch {
  ...
  import_db {../syn_1/pwm_demo_gate.db}
  read_sdc
  place
  export_db {pwm_demo_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file pwm_demo_phy.area
  export_db {pwm_demo_pr.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid pwm_demo_inst.bid
  bitgen -bit "pwm_demo.bit"
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
