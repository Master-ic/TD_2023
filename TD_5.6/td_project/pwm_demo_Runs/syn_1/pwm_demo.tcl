...
set rc [catch {
  open_project {pwm_demo.prj}
  import_device sf1_6.db -package SF1S60CG121I
  elaborate -top {SF1_SOC}
  export_db {pwm_demo_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../pin.adc
  optimize_rtl
  report_area -file pwm_demo_rtl.area
  export_db {pwm_demo_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  read_sdc
  optimize_gate -maparea pwm_demo_gate.area
  legalize_phy_inst
  export_db {pwm_demo_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
