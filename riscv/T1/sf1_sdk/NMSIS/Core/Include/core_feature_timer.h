...
__STATIC_INLINE uint32_t SysTick_Config(uint64_t ticks)
{
    ...
    SysTimer_SetCompareValue(ticks); // Set system timer compare value
    ECLIC_SetShvIRQ(SysTimer_IRQn, ECLIC_NON_VECTOR_INTERRUPT);	// Set interrupt working mode for a specific interrupt
    ECLIC_SetLevelIRQ(SysTimer_IRQn, 1); // Set ECLIC Interrupt level of a specific interrupt
    ECLIC_EnableIRQ(SysTimer_IRQn); // Enable a specific interrupt
    return (0UL);
}
