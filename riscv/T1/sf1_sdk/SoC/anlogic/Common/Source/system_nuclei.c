...
// SYSTEM_CLOCK is the value from osc/gclk(PLL) after divided which can be configed by MISC reg.
// (Clock divide offset_0x10 reg bit[5:1])
#ifndef SYSTEM_CLOCK
    #ifdef  CFG_SIMULATION
        #define  SYSTEM_CLOCK    (32000000UL)
    #else

//		#define   SYSTEM_CLOCK    (260000000UL) // fail
//	    #define   SYSTEM_CLOCK    (200000000UL) // fail
//		#define   SYSTEM_CLOCK    (160000000UL) // fail
//		#define   SYSTEM_CLOCK    (140000000UL) // pass
//		#define   SYSTEM_CLOCK    (120000000UL) // pass
//		#define   SYSTEM_CLOCK    (110000000UL) // pass
		#define   SYSTEM_CLOCK    (100000000UL)	//pass
//		#define   SYSTEM_CLOCK    (90000000UL)	//pass
...
void SystemInit (void)
{
     ...
     */
    SystemCoreClock = SYSTEM_CLOCK;
   // enable i cache, (if in work2 mode,i cache must disable)
...
static void system_default_exception_handler(unsigned long mcause, unsigned long sp)
{
	...
	// anlogic printf
//    anl_printf("MCAUSE: 0x%lx\r\n", mcause);
//    anl_printf("MEPC  : 0x%lx\r\n", __RV_CSR_READ(CSR_MEPC));
//    anl_printf("MTVAL : 0x%lx\r\n", __RV_CSR_READ(CSR_MBADADDR));
...
#endif
    const char* download_modes[] = {"FLASHXIP", "FLASH", "ILM", "WORK1", "WORK2", "WORK3"};
//    printf("Nuclei SDK Build Time: %s, %s\r\n", __DATE__, __TIME__);
//    printf("Download Mode: %s\r\n", download_modes[DOWNLOAD_MODE]);
//    printf("CPU Frequency %lu Hz\r\n", SystemCoreClock);
    anl_printf("Nuclei SDK Build Time: %s, %s\r\n", __DATE__, __TIME__);
    anl_printf("Download Mode: %s\r\n", download_modes[DOWNLOAD_MODE]);
    anl_printf("CPU Frequency %d Hz\r\n", SystemCoreClock);
#endif
}
...
{
//    misc_clock_sel_ocs();
	// choose gclk from PLL
//    misc_clock_sel_divide();  // always use osc_clk in boot stage  Yi

//    misc_set_clock_divide(0); // 1; no need to set the clock divide here cause it is already set in startup.S
//    misc_set_clock_divide(2); // 1/(N+1)
    // misc_ppi_clock_divide(1); // /*F_spi=1/[2(div+1)], default div=0*/
   
   // REG32(0xE0000010) &= ~(0xf<<6);
    /* TODO: Add your own initialization code here, called before main */
//    SystemCoreClock = get_cpu_freq();
//    SystemCoreClock = SystemCoreClock/(misc_get_clock_divide()+1);
    
	/*SystemCoreClock must set as actual value, otherwise uart buad-frequncy not ok*/
    uart_init(SOC_DEBUG_UART, 115200,UART_BIT_LENGTH_8);
  //  misc_set_clock_divide(3);
